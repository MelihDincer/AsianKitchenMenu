const menu = [
  ...
  //BUTTON EKLEME İŞLEMLERİ
  const navButton= document.querySelector(".btn-container");
  var buttons = ['All',...new Set(menu.map(item => item.category))]
   function btnCreate(){
        for(let i=0; i<buttons.length; i++){
        const createBtn = document.createElement("button");
        createBtn.classList.add("btn", "btn-outline-dark", "btn-item");
        createBtn.innerHTML = buttons[i];
        navButton.appendChild(createBtn);
        createBtn.addEventListener("click", e => {
          if (createBtn.innerText === 'All') {
            addMenuItemsToHTML();
          }
          else {
            addMenuItemsToHTML(createBtn.innerText);
          }
        })
    }
  }
  ...
  function addMenuItemsToHTML() {
    ...
    let menuList = [];
      ...
      menuList = menu.filter(item => item.category === arguments[0]);
    } 
